...
const AboutPage: NextPage<AboutPageProps> = ({ data }) => {
  ...
  const { about, work_exp, contact, education, skills, languages, interests } = data;

  if (!data || isEmpty(data)) {
    return <h1>Sorry, this page was expired.</h1>;
  }

  return <Box mb={10} maxW={768} mx="auto">
    <div className={styles.container}>
      ...
      <SimpleGrid columns={{ lg: 2 }}>
        ...
        <Box>
          <Heading as="h2" size="md">{interests.title}</Heading>
          <Flex ml={4} mt={2}>
            {
              interests.content.map((interest, i) =>
                <Text key={i} mr={3} fontSize={22}>{interest}</Text>)
            }
          </Flex>
        </Box>
      </SimpleGrid>
